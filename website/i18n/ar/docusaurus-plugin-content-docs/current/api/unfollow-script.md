---
sidebar_position: 5
title: 取消تكوين برنامج المتابعة النصي
description: 取消مرجع التكوين الكامل لبرنامج المتابعة النصي
---

توضح هذه الصفحة معاملات التكوين لبرنامج `unfollow` النصي المستخدم في إنشاء المهام.

## نظرة عامة

`unfollow` يُستخدم البرنامج النصي لمتابعة المستخدمين تلقائيًا على TikTok أو Instagram 上自动取消关注用户。عند توفير مستخدمين مستهدفين متعددين عبر API،**سينشئ النظام مهمة لكل مستخدم مستهدف**。يمكنك استخدام معامل `start_time` للتحكم في وقت تنفيذ المهمة.

## تكوين البرنامج النصي (`script_config`)

`script_config` يحتوي كائن取消关注النصي المستخدم في إنشاء المهام.فيما يلينعم可用المعاملات：

### المعاملات

| المعاملات | النوع | مطلوب | القيمة الافتراضية | الوصف |
|------|------|------|--------|------|
| target_users | نص[] | نعم* | [] | 要取消关注的目标مصفوفة أسماء المستخدمين（每个用户一个任务） |
...
3. **合理批量处理**：不要一次取消关注太多用户。سينشئ النظام مهمة لكل مستخدم مستهدف，因此大列表会产生许多任务。
