---
sidebar_position: 1
title: TikMatrix/IgMatrix 软件定位与خريطة الطريق
sidebar_label: خريطة الطريق
description: 官网版خريطة الطريق，نظرة عامة TikMatrix/IgMatrix 在自动化运营生态中的定位、能力边界与上线建议。
slug: roadmap
---

## 全流程地图

![TikMatrix/IgMatrix خريطة الطريق](/img/roadmap-zh.svg)
...
1. **可编排的批量自动化**：以“任务 → 脚本 → 数据源”的模型构建可复用流水线，覆盖预热、النشر、互动、采集的全链路动作。
...
- **التسويق الفائق脚本**：已融合 Boost 用户/帖子、群发私信、التعليق الجماعي等能力
- تدفئة الحساب脚本：日常浏览、停留、轻互动
- 内容النشر脚本：فيديو/文案/标签/话题管理，定时النشر
- 数据采集脚本：抓取用户名معلومات，构建下一轮目标清单
...
2. **试点期（10–20 台）**：引入التسويق الفائق脚本 + 数据采集闭环；观察风控阈值
...
- استخدام自动化工具可能违反平台服务条款；请**自担风险**并合理控制频率与行为模式
