---
sidebar_position: 5
title: Unfollow Script Configuratie
description: Volledige configuratiereferentie voor het unfollow script
---

Deze pagina documenteert de configuratieparameters voor het `unfollow` script dat gebruikt wordt bij het aanmaken van taken.

## Overzicht

Het `unfollow` script wordt gebruikt om automatisch gebruikers te ontvolgen op TikTok of Instagram. Wanneer u meerdere doelgebruikers via API aanlevert, **wordt er één taak gemaakt per doelgebruiker**. U kunt bepalen wanneer elke taak wordt uitgevoerd met de `start_time` parameter.

## Script Configuratie (`script_config`)

Het `script_config` object bevat de parameters voor het unfollow script. Hieronder staan de beschikbare parameters:

### Parameters

| Parameter | Type | Verplicht | Standaard | Beschrijving |
|-----------|------|----------|---------|-------------|
| target_users | string[] | Ja* | [] | Array van doel gebruikersnamen om te ontvolgen (één taak per gebruiker) |
...
### Ontvolg Meerdere Gebruikers

Bij het ontvolgen van meerdere gebruikers, wordt er één taak per gebruiker aangemaakt:
...
### Ontvolg Gebruikers via Zoekmethode
...
### Batch Ontvolgen op Meerdere Apparaten
...
3. **Batch verstandig**: Ontvolg niet te veel gebruikers tegelijk. Het systeem maakt één taak per doelgebruiker, dus grote lijsten resulteren in veel taken.
