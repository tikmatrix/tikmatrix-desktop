---
sidebar_position: 5
title: Unfollow-Skript-Konfiguration
description: Vollständige Referenz für die Unfollow-Skript-Konfiguration
---

Auf dieser Seite werden die Konfigurationsparameter für das `unfollow`-Skript beschrieben, das beim Erstellen von Aufgaben verwendet wird.

## Überblick

Das `unfollow`-Skript wird verwendet, um Benutzern auf TikTok oder Instagram automatisch zu entfolgen. Wenn mehrere Zielbenutzer über die API angegeben werden, **erstellt das System eine Aufgabe pro Benutzer**. Sie können die Ausführungszeit von Aufgaben mit dem Parameter `start_time` steuern.

## Skript-Konfiguration (`script_config`)

Das `script_config`-Objekt enthält Parameter für das Unfollow-Skript. Nachfolgend die verfügbaren Parameter:
...
### Mehreren Benutzern entfolgen

Beim Entfolgen mehrerer Benutzer wird für jeden eine separate Aufgabe erstellt:
...
### Batch-Entfolgen auf mehreren Geräten
...
3. **Vernünftige Stapelverarbeitung**: Entfolgen Sie nicht zu vielen Benutzern auf einmal. Das System erstellt eine Aufgabe pro Benutzer, große Listen erzeugen also viele Aufgaben.
