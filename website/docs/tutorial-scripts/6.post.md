...
- **Caption File**: An optional `caption.txt` per username folder for custom captions. Falls back to root `caption.txt` if not found.
  - **Format Description**: Caption.txt only supports Spintax template format (same usage as UI's Spintax mode):
    - Configure one Spintax template using `{option1|option2}` syntax for random variations
    - Use native line breaks (Enter key) for multi-line content
    - Supports `##` title/description split (TikTok only, not Instagram)
